...
#include <stdint.h>
#include <stdio.h>
// ysyxSoC 的 PS/2 控制器返回的是原始 scan code，不再是 KEYDOWN_MASK 编码后的结果
// 因此这里不需要再用 KEYDOWN_MASK 去解包 MMIO 返回值

static uint8_t ps2_ext = 0;    // 是否收到了 0xE0
static uint8_t ps2_break = 0;  // 是否收到了 0xF0

static int ps2_set2_to_am(uint8_t sc, bool ext) {
  if (!ext) {
    switch (sc) {
      // 功能键
...
void __am_input_keybrd(AM_INPUT_KEYBRD_T *kbd) {
  uint8_t sc = inl(KBD_ADDR) & 0xff;
  //printf("scan code read : %x\n",sc);
  // 默认：没有完整事件
  kbd->keydown = false;
  ...
  // 真正的扫描码字节到来，翻译成 AM_KEY_*
  int key = ps2_set2_to_am(sc, ps2_ext);

  if (key != AM_KEY_NONE) {
    kbd->keycode = key;
    kbd->keydown = ps2_break ? false : true;
  }
